                ...
                @foreach ($products as $item)
                    <div class="card-col col mb-4">
                        <div class="card-shape card h-100">
                            <img src="{{ $item->product_img }}" class="card-img-top"
                                style="border-radius: 100%; padding:20px;" alt="...">
                            <div class="card-body">
                                <a href="{{ url('addtowishlist') }}/{{ $item->id }}"
                                    class="card-heart-btn btn btn-outline-success"><i class="far fa-heart"></i><span
                                        class="heart-text">Add to whishlist</span></a>
                                <h4 class="card-title">{{ $item->product_name }}</h4>
                                <p class="card-text">{{ $item->description }}</p>
                                <h2 class="price">{{ $item->price }} $</h2>
                                <a href="{{ url('addtocart') }}/{{ $item->id }}"
                                    class="card-btn btn btn-outline-success">Order now</a>
                            </div>
                        </div>
                    </div>
                @endforeach
            @else
                <div style="margin-left: auto; margin-right: auto; text-align: center; padding-top:20px; background-color: #ff00003d;">
